...
$("#lookupServices div.ui-accordion-content-active ul li.selected").data())for(var e in d)e.match(/jQuery/)&&delete d[e];else return $("#lookupServices div.ui-accordion-content-active div.searchResultsParent").css({borderColor:"red"}),!1;d&&(d.certainty=$("#certainty input:checked").val())}1==i&&null!=d?a.editEntity(a.editor.currentEntity,d):a.finalizeEntity(b,d);h.dialog("close");b=null};return{show:function(c){b=c.type;i=c.entry?1:0;var g="Tag ",l;1==i?(g="Edit ",l=a.entities[a.editor.currentEntity].props.content):
l=a.editor.currentBookmark.rng.toString();$("#lookupServices div.searchResultsParent").css({borderColor:"#fff"}).children("ul").empty();$('#lookup_alternate input[type="text"]').css({borderColor:"#ccc"}).val("");d.value=l;h.dialog("option","title",g+c.title);h.dialog("option","buttons",[{text:"Cancel",click:function(){j(!0)}},{text:"Add New "+c.title,click:function(){a.d.show("add"+b,{})}},{text:"Tag "+c.title,click:function(){j()}}]);c.pos?h.dialog("option","position",[c.pos.x,c.pos.y]):h.dialog("option",
"position","center");h.dialog("open");$("#lookupServices").accordion("resize");1==i?($('#certainty input[value="'+c.entry.info.certainty+'"]').click(),c.entry.info.type&&"alt_id"==c.entry.info.type?($("#lookupServices").accordion("activate",3),$('#lookup_alternate input[name="'+c.entry.info.typeName+'"]').val(c.entry.info.value).prevAll("input").click()):$("#lookupServices").accordion("activate",0)):($("#c_definite").trigger("click"),0==$("#lookupServices").accordion("option","active")?e():$("#lookupServices").accordion("activate",
0))},hide:function(){h.dialog("close")}}},SettingsDialog=function(g,a){var b={fontSize:"11pt",fontFamily:"Book Antiqua",showEntityBrackets:!1,showStructBrackets:!1};jQuery.extend(b,a);$("#header").append('<div id="helpLink"><img src="img/help.png" title="Help"/><h2>Help</h2></div><div id="settingsLink"><h2>Settings</h2></div>');$(document.body).append('<div id="settingsDialog"><div><label>Font Size</label><select name="fontsize"><option value="9pt">9pt</option><option value="10pt">10pt</option><option value="11pt">11pt</option><option value="12pt">12pt</option><option value="13pt">13pt</option></select></div><div style="margin-top: 10px;"><label>Font Type</label><select name="fonttype"><option value="Arial" style="font-family: Arial; font-size: 12px;">Arial</option><option value="Book Antiqua" style="font-family: Book Antiqua; font-size: 12px;">Book Antiqua</option><option value="Georgia" style="font-family: Georgia; font-size: 12px;">Georgia</option><option value="Helvetica" style="font-family: Helvetica; font-size: 12px;">Helvetica</option><option value="Palatino" style="font-family: Palatino; font-size: 12px;">Palatino</option><option value="Tahoma" style="font-family: Tahoma; font-size: 12px;">Tahoma</option><option value="Times New Roman" style="font-family: Times New Roman; font-size: 12px;">Times New Roman</option><option value="Verdana" style="font-family: Verdana; font-size: 12px;">Verdana</option></select></div><div style="margin-top: 10px;"><label for="showentitybrackets">Show Entity Brackets</label><input type="checkbox" id="showentitybrackets" /></div><div style="margin-top: 10px;"><label for="showstructbrackets">Show Tags</label><input type="checkbox" id="showstructbrackets" /></div><div style="margin-top: 10px;"><label>Editor Mode</label><select name="editormode"><option value="0">XML & RDF (overlapping entities)</option><option value="1">XML (no overlap)</option></select></div><div style="margin-top: 10px;"><label>Schema</label><select name="schema" disabled="disabled"><option value="cwrcbasic">CWRC Basic TEI Schema</option><option value="events">Events Schema</option></select></div></div><div id="helpDialog"></div>');
$("#settingsLink").click(function(){$('select[name="fontsize"] > option[value="'+b.fontSize+'"]',$("#settingsDialog")).attr("selected",!0);$('select[name="fonttype"] > option[value="'+b.fontFamily+'"]',$("#settingsDialog")).attr("selected",!0);$("#showentitybrackets").prop("checked",b.showEntityBrackets);$("#showstructbrackets").prop("checked",b.showStructBrackets);$('select[name="editormode"] > option[value="'+g.mode+'"]',$("#settingsDialog")).attr("selected",!0);$('select[name="schema"] > option[value="'+
g.validationSchema+'"]',$("#settingsDialog")).attr("selected",!0);$("#settingsDialog").dialog("open")});$("#helpLink").click(function(){0==$("#helpDialog iframe").length&&$("#helpDialog").html('<iframe src="https://sites.google.com/site/cwrcwriterhelp/home"/>');$("#helpDialog").dialog("open")});$("#settingsDialog").dialog({title:"Settings",modal:!0,resizable:!1,closeOnEscape:!0,height:270,width:325,autoOpen:!1,buttons:{Ok:function(){var a=parseInt($('select[name="editormode"]',$("#settingsDialog")).val());
if(a!=g.mode){var h=!0,d;if(d=g.mode==g.XMLRDF&&a==g.XML)a:{g.highlightEntity();for(var e in g.entities){var c=g.editor.dom.select('entity[name="'+e+'"]');d=c[1];for(c=c[0];c!=d&&null!=c;)if(c=c.nextSibling,"entity"==c.nodeName.toLowerCase()&&c!=d){d=!0;break a}}d=!1}d&&(h=!1,g.d.show("message",{title:"Error",msg:"You have overlapping entities and are trying to change to XML mode (which doesn't permit overlaps).  Please remove the overlapping entities and try again.",type:"error"}));h&&(g.mode=a)}b.fontSize=
$('select[name="fontsize"]',$("#settingsDialog")).val();b.fontFamily=$('select[name="fonttype"]',$("#settingsDialog")).val();b.showEntityBrackets!=$("#showentitybrackets").prop("checked")&&g.editor.$("body").toggleClass("showEntityBrackets");b.showEntityBrackets=$("#showentitybrackets").prop("checked");b.showStructBrackets!=$("#showstructbrackets").prop("checked")&&g.editor.$("body").toggleClass("showStructBrackets");b.showStructBrackets=$("#showstructbrackets").prop("checked");g.validationSchema=
$('select[name="schema"]',$("#settingsDialog")).val();a={fontSize:b.fontSize,fontFamily:b.fontFamily};g.editor.dom.setStyles(g.editor.dom.getRoot(),a);$("#settingsDialog").dialog("close")},Cancel:function(){$("#settingsDialog").dialog("close")}}});$("#helpDialog").dialog({title:"Help",modal:!0,resizable:!0,closeOnEscape:!0,height:500,width:900,autoOpen:!1,buttons:{Close:function(){$("#helpDialog").dialog("close")}}});return{getSettings:function(){return b}}},TitleDialog=function(g){var a=g.writer,
...
var i=$("#titleDialog");i.dialog({modal:!0,resizable:!1,closeOnEscape:!1,open:function(){$("#titleDialog").parent().find(".ui-dialog-titlebar-close").hide()},height:310,width:435,autoOpen:!1,buttons:{"Tag Text/Title":function(){h()},Cancel:function(){h(!0)}}});$("#titleDialog input").keyup(function(a){"13"==a.keyCode&&(a.preventDefault(),h())});var h=function(d){var e=null;if(!d)var d=$('input[name="level"]:checked',i).val(),e=$('input[name="ref"]',i).val(),c=$('input[name="alt"]',i).val(),h=$('input[name="unformatted"]',
i).prop("checked"),e={level:d,ref:e,alt:c,unformatted:h};1==b&&null!=e?a.editEntity(a.editor.currentEntity,e):a.finalizeEntity("title",e);i.dialog("close")};return{show:function(a){b=a.entry?1:0;var e="Tag ";if(0==b)$('input[value="m"]',i).attr("checked",!0),$('input[name="ref"]',i).attr("value",""),$('input[name="alt"]',i).attr("value",""),$('input[name="unformatted"]',i).attr("checked",!1);else{var e="Edit ",c=a.entry.info.ref,h=a.entry.info.alt,g=a.entry.info.unformatted;$('input[value="'+a.entry.info.level+
'"]',i).attr("checked",!0);$('input[name="ref"]',i).attr("value",c);$('input[name="alt"]',i).attr("value",h);$('input[name="unformatted"]',i).attr("checked",g)}i.dialog("option","title",e+a.title);a.pos?i.dialog("option","position",[a.pos.x,a.pos.y]):i.dialog("option","position","center");i.dialog("open")},hide:function(){i.dialog("close")}}},TripleDialog=function(g){var a=g.writer,b={person:["is a child of","is a parent of","is related to","was born on","died on"],place:["is located within","contains"]};
...
var i=$("#tripleDialog");i.dialog({title:"Add Relation",modal:!0,resizable:!0,closeOnEscape:!0,height:450,width:600,autoOpen:!1,buttons:{Close:function(){i.dialog("close")}}});$("#subjectColumn input").watermark("Custom Subject");$("#predicateColumn input").watermark("Custom Predicate");$("#objectColumn input").watermark("Custom Object");$("#tripleColumnsParent input").keyup(function(){$(this).parents(".column").find("li").removeClass("selected");e()});$("#currentRelation button").button({disabled:!0}).click(function(){var b=
d(),e=b[0],g=b[1],b=b[2],i=tinymce.DOM.uniqueId("tri_");a.triples.push({id:i,subject:e,predicate:{text:g.text,name:h(g.text),external:g.external},object:b});a.relations.update()});var h=function(a){for(var a=a.split(/\s/),b="",d=0;d<a.length;d++)b=0==d?b+a[d].toLowerCase():b+a[d].replace(/^./,function(a){return a.toUpperCase()});return b},d=function(){var b=[null,null,null];$("#tripleColumnsParent ul").each(function(d){var e=$(this).find(".selected");1==e.length&&(b[d]={text:a.u.escapeHTMLString(e.text()),
uri:e.attr("name"),external:!1})});$("#tripleColumnsParent input").each(function(d){var e=$(this).val();""!=e&&(b[d]={text:a.u.escapeHTMLString(e),uri:a.u.escapeHTMLString(e),external:!0})});return b},e=function(){for(var a="",b=d(),e=!0,h=0;h<b.length;h++){var g=b[h];null==g?e=!1:(a+=g.text,a=2>h?a+" ":a+".")}$("#currentRelation p").text(a);e?$("#currentRelation button").button("enable"):$("#currentRelation button").button("disable")};return{show:function(){$("#subjectColumn > ul, #predicateColumn > ul, #objectColumn > ul").empty();
$("#currentRelation p").html("");var c="",d;for(d in a.entities)var h=a.entities[d],c=c+('<li class="'+h.props.type+'" name="'+h.props.id+'"><span class="box"/><span class="entityTitle">'+h.props.title+"</span></li>");$("#subjectColumn > ul, #objectColumn > ul").html(c);$("#tripleDialog .entitiesList > li").hover(function(){$(this).hasClass("selected")||$(this).addClass("over")},function(){$(this).hasClass("selected")||$(this).removeClass("over")}).click(function(){$(this).siblings("li").removeClass("selected");
$(this).removeClass("over").toggleClass("selected");$(this).parents(".column").find("input").val("");if(0<$(this).parents("#subjectColumn").length)if($(this).hasClass("selected")){var c=a.entities[$(this).attr("name")].props.type;$("#predicateColumn > ul").empty();for(var c=b[c]||["is associated with"],d="",h=0;h<c.length;h++)d+="<li>"+c[h]+"</li>";$("#predicateColumn > ul").html(d);$("#predicateColumn ul li").click(function(){$(this).siblings("li").removeClass("selected");$(this).toggleClass("selected");
$(this).parents(".column").find("input").val("");e()})}else $("#predicateColumn > ul").empty();e()});i.dialog("open")},hide:function(){i.dialog("close")}}};var DialogManager=function(g){var a=null,b={message:new MessageDialog(g),search:new SearchDialog(g),note:new NoteDialog(g),citation:new CitationDialog(g),correction:new CorrectionDialog(g),keyword:new KeywordDialog(g),title:new TitleDialog(g),date:new DateDialog(g),link:new LinkDialog(g),addperson:new AddPersonDialog(g),addplace:new AddPlaceDialog(g),addevent:new AddEventDialog(g),addorg:new AddOrganizationDialog(g),triple:new TripleDialog(g),header:new HeaderDialog(g)};b.person=b.search;b.place=
b.search;b.event=b.search;b.org=b.search;return{getCurrentType:function(){return a},show:function(g,h){b[g]&&(a=g,b[g].show(h))},confirm:function(g){a="message";b.message.confirm(g)},hideAll:function(){for(var a in b)b[a].hide()}}};var Utilities=function(g){var a=g.writer,g={getTitleFromContent:function(a){return 34>=a.length?a:a.substring(0,34)+"&#8230;"},escapeHTMLString:function(a){return"string"==typeof a?a.replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/'/g,"&#039;").replace(/</g,"&lt;").replace(/>/g,"&gt;"):a},unescapeHTMLString:function(a){return"string"==typeof a?a.replace(/&quot;/g,'"').replace(/&#039;/g,"'").replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&amp;/g,"&"):a},isSelectionValid:function(b){function d(a,
b,c){if(20<c)return null;a="back"==b?a.lastChild||a.previousSibling||a.parentNode.previousSibling:a.firstChild||a.nextSibling||a.parentNode.nextSibling;return null==a?null:a.nodeType==Node.TEXT_NODE?a:d(a,b,c++)}var e=a.editor.selection,c=e.getRng(!0);if("body"==c.commonAncestorContainer.nodeName.toLowerCase()){var g=a.editor.$("body > *")[0];c.setStartBefore(g.firstChild);c.setEndAfter(g.lastChild)}if(!b){if(c.startContainer.nodeType==Node.ELEMENT_NODE){g=c.endContainer;if(g.nodeType!=Node.TEXT_NODE||
0==c.endOffset){g=d(c.endContainer,"back",0);if(null==g)return a.NO_COMMON_PARENT;c.setEnd(g,g.length)}c.setStart(g,0)}c.endContainer.nodeType==Node.ELEMENT_NODE&&c.setEnd(c.startContainer,c.startContainer.length)}var i=function(a,b,c){if(0<b&&20>c){a.startOffset<a.startContainer.length&&(a.setStart(a.startContainer,a.startOffset+1),b--);if(a.startOffset==a.startContainer.length){var e=d(a.startContainer,"forward",0);null!=e&&a.setStart(e,0)}i(a,b,c++)}},m=function(a,b,c){if(0<b&&20>c){0<a.endOffset&&
(a.setEnd(a.endContainer,a.endOffset-1),b--);if(0==a.endOffset){var e=d(a.endContainer,"back",0);null!=e&&a.setEnd(e,e.length)}m(a,b,c++)}},g=c.toString(),l=g.match(/^\s+/),f=0,p=0;null!=l&&(f=l[0].length);l=g.match(/\s+$/);null!=l&&(p=l[0].length);i(c,f,0);m(c,p,0);e.setRng(c);if(c.startContainer.parentNode!=c.endContainer.parentNode)if(0==c.endOffset&&c.endContainer.previousSibling==c.startContainer.parentNode)c.setEnd(c.startContainer,c.startContainer.length);else return a.NO_COMMON_PARENT;if(b||
a.mode==a.XML){g=c.startContainer;for(b={};g!=c.endContainer;)if(g=g.nextSibling,e=$(g),e.hasClass("entity"))if(e.hasClass("start"))b[e.attr("name")]=!0;else if(b[e.attr("name")])delete b[e.attr("name")];else return a.NO_COMMON_PARENT;var c=0,y;for(y in b)c++;if(0!=c)return a.NO_COMMON_PARENT}return a.VALID}},b=function(a,d,e,c,g){a.find(c).each(function(a,b){var d=$(b);if(!(0<d.parents("element").length&&0<e)){var h={name:d.attr("name"),level:e+0};h[c]=d;g.push(h)}});a.find("ref").each(function(a,
h){var i=$(h).attr("name");!(0<$(h).parents("element").length&&0<e)&&!d[i]&&(d[i]=!0,i=$('define[name="'+i+'"]',writer.schemaXML),b(i,d,e+1,c,g))})};g.getChildrenForTag=function(a){var d=$('element[name="'+a.tag+'"]',writer.schemaXML),e=[];b(d,{},0,a.type||"element",e);if("array"==a.returnType)return e.sort(function(a,b){return a.level-b.level}),e;a={};for(d=0;d<e.length;d++){var c=e[d];a[c.name]=c}return a};var i=function(a,b,e,c){$('define:has(ref[name="'+a+'"])',writer.schemaXML).each(function(a,
h){var g=$(h).attr("name");if(!b[g]){b[g]=!0;var l=$(h).find("element").first();1==l.length?c[l.attr("name")]={name:l.attr("name"),level:e+0}:i(g,b,e+1,c)}})};g.getParentsForTag=function(a){var a=$('element[name="'+a+'"]',writer.schemaXML).parents("define").attr("name"),b={};i(a,{},0,b);return b};return g};var FileManager=function(g){function a(a){var c=[],d=a.attr("id"),a=a.attr("_tag")||a.attr("_type"),e="<"+a,f=b.structs[d],d=b.entities[d];if(f)for(var h in f)0!=h.indexOf("_")&&(d=h,"id"==d&&(d=b.idName),e+=" "+d+'="'+f[h]+'"');else if(d)for(h in d.info)e+=" "+h+'="'+d.info[h]+'"';c.push(e+">");c.push("</"+a+">");return c}var b=g.writer,i=null,h=[];$(document.body).append('<div id="loader"><div id="files"><ul class="searchResults"></ul></div></div><div id="saver"><label for="filename">Name</label><input type="text" name="filename"/><p>Please enter letters only.</p></div><div id="entitiesConverter"></div><div id="editSourceDialog"><textarea style="width: 100%; height: 98%;"></textarea></div><iframe id="editDocLoader" style="display: none;"></iframe>');
var d=$("#loader");d.dialog({title:"Load Document",modal:!0,height:450,width:300,autoOpen:!1,buttons:{Load:function(){var a=$("#files ul li.selected").data();a?(j.loadDocument(a.name),d.dialog("close")):$("#files").css({borderColor:"red"})},Cancel:function(){d.dialog("close")}}});var e=$("#saver");e.dialog({title:"Save Document As",modal:!0,resizable:!1,height:150,width:300,autoOpen:!1,buttons:{Save:function(){var a=$("#saver input").val();null!=a.match(/[^A-Za-z]+/)?b.d.show("message",{title:"Invalid Name",
msg:"You may only enter upper or lowercase letters; no numbers, spaces, or punctuation.",type:"error"}):"info"==a?b.d.show("message",{title:"Invalid Name",msg:"This name is reserved, please choose a different one.",type:"error"}):-1!=$.inArray(a,h)?b.d.show("message",{title:"Invalid Name",msg:"This name already exists, please choose a different one.",type:"error"}):(i=a,j.validate(!0),e.dialog("close"))},Cancel:function(){e.dialog("close")}}});var c=$("#editSourceDialog");c.dialog({title:"Edit Source",
modal:!0,resizable:!0,closeOnEscape:!0,height:480,width:640,autoOpen:!1,buttons:{Ok:function(){b.d.show("message",{title:"Edit Source",msg:"Edit Source does not currently modify the underlying document.",type:"info"});c.dialog("close")},Cancel:function(){c.dialog("close")}}});var j={openLoader:function(){$("#files").css({borderColor:"#fff"});k(function(){var a="",b="",c,e;for(e=0;e<h.length;e++)c=h[e],b=e==h.length-1?"last":"",a+='<li class="unselectable '+b+'">',a+="<span>"+c+"</span>",a+="</li>";
$("#files ul").first().html(a);$("#files ul li").each(function(a){$(this).data({name:h[a]})});$("#files ul li").click(function(){$("#files").css({borderColor:"#fff"});var a=$(this).hasClass("selected");$("#files ul li").removeClass("selected");a||$(this).addClass("selected")});$("#files ul li").dblclick(function(){$("#files ul li").removeClass("selected");$(this).addClass("selected");j.loadDocument($(this).data("name"));d.dialog("close")});d.dialog("open")})},openSaver:function(){k();e.dialog("open")}},
k=function(a){$.ajax({url:b.baseUrl+"editor/documents",type:"GET",dataType:"json",success:[function(a){h=a},a],error:function(){b.d.show("message",{title:"Error",msg:"Error getting documents.",type:"error"})}})};j.validate=function(a){var c=m(!1);$.ajax({url:b.baseUrl+"services/validator/validate.html",type:"POST",dataType:"XML",data:{sch:"http://cwrc.ca/schema/"+b.validationSchema,type:"RNG_XML",content:c},success:function(c){var d=i;null==d&&(d="The current document");if("pass"!=$("status",c).text()){var e=
"<ul>";$("error",c).each(function(){e+="<li>"+$(this).find("message").text()+"</li>"});e+="</ul>";a?b.d.confirm({title:"Document Invalid",msg:d+" is not valid. <b>Save anyways?</b><br/>"+e,callback:function(a){a&&j.saveDocument()}}):b.d.show("message",{title:"Document Invalid",msg:d+" is not valid.<br/>"+e,type:"error"})}else if(a)j.saveDocument();else{var h="";0<$("warning",c).length&&(h+="<ul>",$("warning",c).each(function(){h+="<li>"+$(this).find("message").text()+"</li>"}),h+="</ul>");b.d.show("message",
{title:"Document Valid",msg:d+" is valid.<br/>"+h,type:"info"})}},error:function(){b.d.show("message",{title:"Error",msg:"An error occurred while trying to validate "+i+".",type:"error"})}})};j.saveDocument=function(){if(null==i)j.openSaver();else{var a=m(!0);$.ajax({url:b.baseUrl+"editor/documents/"+i,type:"PUT",dataType:"json",data:a,success:function(){b.editor.isNotDirty=1;b.d.show("message",{title:"Document Saved",msg:i+" was saved successfully."})},error:function(){b.d.show("message",{title:"Error",
msg:"An error occurred and "+i+" was not saved.",type:"error"})}})}};j.buildXMLString=function(b){function c(b){var e=a(b);d+=e[0];b.contents().each(function(a,b){1==b.nodeType?c($(b)):3==b.nodeType&&(d+=b.data)});d+=e[1]}var d="";c(b);return d};var m=function(c){b.highlightEntity();var d='<?xml version="1.0" encoding="UTF-8"?>\n',e=$(b.editor.getBody()),h=e.clone(!1,!0);l(e);var f="";if(c){var f='\n<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:w="http://cwrctc.artsrn.ualberta.ca/#">'+
('\n<rdf:Description rdf:about="'+(b.baseUrl+"editor/documents/"+i)+'">\n\t<w:mode>'+b.mode+"</w:mode>\n</rdf:Description>"),g,k=function(a){a.contents().each(function(){var a=$(this);if(this.nodeType==Node.TEXT_NODE&&" "!=this.data)E+=this.length;else if(a.is(b.root)||a.attr("_tag")){var c=a.attr("id");F.push({id:c,offset:E,length:a.text().length});k(a)}else a.attr("_entity")&&a.hasClass("start")&&(c=a.attr("name"),F.push({id:c,offset:E,length:b.entities[c].props.content.length,entity:!0}))})},E=
0,F=[];k(e);g=F;for(var z={},m=[],r=0;r<g.length;r++){var s=g[r];s.entity&&(m.push(s),z[s.id]={contains:[],overlaps:[]})}s=m.length;for(r=0;r<s;r++)for(var p=m[r],B=p.offset+p.length,x=z[p.id],C=0;C<s;C++){var v=m[C],D=v.offset+v.length;p.offset<v.offset&&B>D?x.contains.push(v.id):p.offset<v.offset&&B>v.offset&&B<D?x.overlaps.push(v.id):p.offset>v.offset&&B>D&&D>p.offset?x.overlaps.push(v.id):p.offset<v.offset&&(B<D&&B>v.offset)&&x.overlaps.push(v.id)}for(m=0;m<g.length;m++){var r=g[m],f=f+('\n<rdf:Description rdf:ID="'+
r.id+'">'),u;for(u in r)f+="\n\t<w:"+u+' type="offset">'+r[u]+"</w:"+u+">";if(r.entity){s=b.entities[r.id];f+='\n\t<w:type type="props">'+s.props.type+"</w:type>";f+='\n\t<w:content type="props">'+s.props.content+"</w:content>";for(u in s.info)f+="\n\t<w:"+u+' type="info">'+s.info[u]+"</w:"+u+">";r=z[r.id];for(s=0;s<r.contains.length;s++)f+="\n\t<w:contains>"+r.contains[s]+"</w:contains>";for(s=0;s<r.overlaps.length;s++)f+="\n\t<w:overlaps>"+r.overlaps[s]+"</w:overlaps>"}f+="\n</rdf:Description>"}for(m=
0;m<b.triples.length;m++)g=b.triples[m],f+='\n<rdf:Description rdf:about="'+g.subject.uri+'" w:external="'+g.subject.external+'">\n\t<w:'+g.predicate.name+' w:text="'+g.predicate.text+'" w:external="'+g.predicate.external+'">\n\t\t<rdf:Description rdf:about="'+g.object.uri+'" w:external="'+g.object.external+'" />\n\t</w:'+g.predicate.name+">\n</rdf:Description>";f+="\n</rdf:RDF>\n"}if(b.mode==b.XMLRDF)c&&e.find("[_entity]").remove();else for(var A in b.entities){c=b.editor.dom.select('[name="'+A+
'"]');u=c[0];z=c[1];for(g=[u];u!=z&&null!=u;)u=u.nextSibling,g.push(u);z=' id="'+A+'" _type="'+b.entities[A].props.type+'"';b.editor.$(g).wrapAll("<entity"+z+"/>");b.editor.$(c).remove()}A=e.children(b.root);"TEI"==b.root&&A.attr("xmlns","http://www.tei-c.org/ns/1.0");c=a(A);d+=c[0];d+=f;A.contents().each(function(a,b){1==b.nodeType?d+=j.buildXMLString($(b)):3==b.nodeType&&(d+=b.data)});d+=c[1];e.replaceWith(h);return d},l=function(a){$(a).contents().each(function(a,b){b.nodeType==Node.TEXT_NODE?
b.nodeValue.match(/&.+?;/gim)&&($("#entitiesConverter")[0].innerHTML=b.nodeValue,b.nodeValue=$("#entitiesConverter")[0].innerText||$("#entitiesConverter")[0].firstChild.nodeValue):b.nodeType==Node.ELEMENT_NODE&&l(b)})};j.loadDocumentFromUrl=function(a){i=a;b.entities={};b.structs={};b.triples=[];$.ajax({url:a,type:"GET",success:f,error:function(c,d){i=null;b.d.show("message",{title:"Error",msg:"An error ("+d+") occurred and "+a+" was not loaded.",type:"error"})},dataType:"xml"})};j.loadDocument=function(a){i=
a;b.entities={};b.structs={};b.triples=[];$.ajax({url:b.baseUrl+"editor/documents/"+a,type:"GET",success:f,error:function(c,d){i=null;b.d.show("message",{title:"Error",msg:"An error ("+d+") occurred and "+a+" was not loaded.",type:"error"})},dataType:"xml"})};j.buildEditorString=function(a){function c(a){var e=a.nodeName;d=e==b.root?d+("<"+e.toLowerCase()):d+"<span";d+=' _tag="'+e+'"';var f=$(a).attr(b.idName);null==f&&(f=tinymce.DOM.uniqueId("struct_"),d+=' id="'+f+'"');var h=parseInt(f.split("_")[1]);
h>tinymce.DOM.counter&&(tinymce.DOM.counter=h);b.structs[f]={id:f,_tag:e};$(a.attributes).each(function(a,c){var e=c.name;e==b.idName&&(e="id");b.structs[f][e]=c.value;if("id"==e||null!=e.match(/^_/))d+=" "+e+'="'+c.value+'"'});d+=">";$(a).contents().each(function(a,b){1==b.nodeType?c(b):3==b.nodeType&&(d+=b.data)});d=e==b.root?d+("</"+e.toLowerCase()+">"):d+"</span>"}var d="";c(a);return d};var f=function(a){function c(){var d=[],e=$(a).find("rdf\\:RDF, RDF"),f;f=parseInt(e.find("w\\:mode, mode").first().text())==
b.XML?b.XML:b.XMLRDF;b.mode!=f&&(b.d.show("message",{title:"Editor Mode changed",msg:"The Editor Mode ("+(b.mode==b.XML?"XML only":"XML & RDF")+") has been changed to match the Document Mode ("+(f==b.XML?"XML only":"XML & RDF")+").",type:"info"}),b.mode=f);if(f==b.XMLRDF)e.children().each(function(){if($(this).attr("rdf:ID")){var a=$(this).find("w\\:id, id").text(),c=parseInt(a.split("_")[1]);c>tinymce.DOM.counter&&(tinymce.DOM.counter=c);d.push({id:a,parent:$(this).find("w\\:parent, parent").text(),
offset:parseInt($(this).find("w\\:offset, offset").text()),length:parseInt($(this).find("w\\:length, length").text())});b.entities[a]={props:{id:a},info:{}};$(this).children('[type="props"]').each(function(){var c=$(this)[0].nodeName.split(":")[1].toLowerCase();if("content"==c){var d=b.u.getTitleFromContent($(this).text());b.entities[a].props.title=d}b.entities[a].props[c]=$(this).text()});$(this).children('[type="info"]').each(function(){var c=$(this)[0].nodeName.split(":")[1].toLowerCase();b.entities[a].info[c]=
$(this).text()})}else{var c=$(this),e=c.attr("rdf:about"),f=$(this).children().first(),h=$(this).find("rdf\\:Description, Description"),g=h.attr("rdf:about"),c={subject:{uri:e,text:"false"==c.attr("w:external")?b.entities[e].props.title:e,external:c.attr("w:external, external")},predicate:{text:f.attr("w:text"),name:f[0].nodeName.split(":")[1].toLowerCase(),external:f.attr("w:external")},object:{uri:g,text:"false"==h.attr("w:external")?b.entities[g].props.title:g,external:h.attr("w:external")}};b.triples.push(c)}}),
$(a).find("rdf\\:RDF, RDF").remove();else{var h=function(a,c){var d=0;a.contents().each(function(){if(this.nodeType==Node.TEXT_NODE)d+=this.length;else if(null!=b.titles[this.nodeName.toLowerCase()]){var e=$(this),f=e.attr(b.idName);null==f&&(f=tinymce.DOM.uniqueId("ent_"));c.push({id:f,parent:$(a).attr(b.idName),offset:d,length:e.text().length});var g=e.text();b.entities[f]={props:{id:f,type:this.nodeName.toLowerCase(),content:g,title:b.u.getTitleFromContent(g)},info:{}};$(this.attributes).each(function(a,
c){b.entities[f].info[c.name]=c.value});e.contents().unwrap()}else h($(this),c)})};h($(a.firstChild),d)}e=a.getElementsByTagName(b.root)[0];e=j.buildEditorString(e);b.editor.setContent(e);b.editor.focus();var g,i,k,l,n,q,m,p;for(f=0;f<d.length;f++){p=m=null;q=n=0;g=d[f];e=g.id;if(""!=g.parent)i=b.editor.$("#"+g.parent),i=i.contents().filter(function(){return this.nodeType==Node.TEXT_NODE}),n=g.offset,k=0,l=!1,m=i.filter(function(){if(!l){k+=this.length;if(k>g.offset)return l=!0;n-=this.length}return!1})[0],
q=g.offset+g.length,k=0,l=!1,p=i.filter(function(){if(!l){k+=this.length;if(k>=g.offset+g.length)return l=!0;q-=this.length}return!1})[0];else{i=$(b.editor.getDoc().body);var x=0,C=function(a){a.contents().each(function(){this.nodeType==Node.TEXT_NODE&&" "!=this.data?(x+=this.length,x>g.offset&&null==m&&(m=this,n=g.offset-(x-this.length)),x>=g.offset+g.length&&null==p&&(p=this,q=n+g.length)):C($(this));if(null!=m&&null!=p)return!1})};C(i)}null!=m&&null!=p&&(i=b.editor.selection.getRng(!0),i.setStart(m,
n),i.setEnd(p,q),b.insertBoundaryTags(e,b.entities[e].props.type,i))}b.entitiesList.update();b.tree.update(!0);b.relations.update()}var d;0<a.getElementsByTagName("EVENTS").length?(d="events",b.idName="ID"):(d="tei",b.idName="xml:id");d!=b.root.toLowerCase()?"events"==d?j.loadSchema("xml/orlando_common_and_events_schema.xml",!1,c):j.loadSchema("xml/CWRCBasicTEI.xml",!1,c):c()};j.editSource=function(){var a=m(!0);$("textarea",c).val(a);c.dialog("open")};j.xmlToString=function(a){var b="";try{b=window.ActiveXObject?
a.xml:(new XMLSerializer).serializeToString(a)}catch(c){alert(c)}return b};j.loadSchema=function(a,c,d){$.ajax({url:a,success:function(a){b.schemaXML=a;var a=$("start",b.schemaXML).find("ref, element").first(),e=a.attr("name");b.root=a.is("element")?e:$('define[name="'+e+'"]',b.schemaXML).find("element").first().attr("name");b.editor.settings.forced_root_block=b.root;b.editor.schema.addCustomElements(b.root);b.editor.schema.addCustomElements(b.root.toLowerCase());"events"==b.root.toLowerCase()?(a=
"css/orlando.css",b.validationSchema="events",b.header="ORLANDOHEADER",b.idName="ID"):(a="css/tei.css",b.validationSchema="cwrcbasic",b.header="teiHeader",b.idName="xml:id");$("#schemaTags",b.editor.dom.doc).remove();$("#schemaRules",b.editor.dom.doc).remove();j.loadSchemaCSS(a);$("head",b.editor.dom.doc).append('<style id="schemaTags" type="text/css" />');var a=b.root,f=b.root.toLowerCase()+" { display: block; }",f=f+(".showStructBrackets "+b.root.toLowerCase()+':before { color: #aaa; font-weight: normal; font-style: normal; font-family: monospace; content: "<'+
a+'>"; }'),f=f+(".showStructBrackets "+b.root.toLowerCase()+':after { color: #aaa; font-weight: normal; font-style: normal; font-family: monospace; content: "</'+a+'>"; }'),h=[];$("element",b.schemaXML).each(function(a,b){var c=$(b).attr("name");-1==h.indexOf(c)&&(h.push(c),f+=".showStructBrackets span[_tag="+c+']:before { color: #aaa; font-weight: normal; font-style: normal; font-family: monospace; content: "<'+c+'>"; }',f+=".showStructBrackets span[_tag="+c+']:after { color: #aaa; font-weight: normal; font-style: normal; font-family: monospace; content: "</'+
c+'>"; }')});f+="span[_tag="+b.header+"] { display: none !important; }";$("#schemaTags",b.editor.dom.doc).text(f);b.schema.elements=h;a="";c&&(a="Paste or type your text here.");b.editor.setContent("<"+b.root+' _tag="'+b.root+'">'+a+"</"+b.root+">");b.entitiesList.update();b.tree.update(!0);b.relations.update();d&&d()},error:function(a,c,d){b.d.show("message",{title:"Error",msg:"Error loading schema: "+d,type:"error"})}})};j.loadSchemaCSS=function(a){function c(){for(var a=null,e=b.editor.getDoc().styleSheets,
f=0;f<e.length;f++){var h=e[f];if(h.href&&-1!=h.href.indexOf(d)){a=h;break}}if(a)try{$("#schemaRules",b.editor.dom.doc).remove();for(var g=a.cssRules,e="",f=0;f<g.length;f++)var i=g[f].selectorText,j=i.replace(/(^|,|\s)(\w+)/g,function(a,b,c){return b+'span[_tag="'+c+'"]'}),k=g[f].cssText.replace(i,j),e=e+(k+"\n");$("head",b.editor.dom.doc).append('<style id="schemaRules" type="text/css" />');$("#schemaRules",b.editor.dom.doc).text(e);a.disabled=!0}catch(l){setTimeout(c,25)}else setTimeout(c,25)}
b.editor.dom.loadCSS(a);var d=a.split("/"),d=d[d.length-1],e=b.editor.getDoc().styleSheets.length,f=null;0<e?c():f=setInterval(function(){b.editor.getDoc().styleSheets.length>e&&(clearInterval(f),c())},25)};j.loadInitialDocument=function(a){a.match("load")?j.openLoader():a.match("sample_letter")?p("xml/sample_letter.xml"):a.match("sample_poem")?p("xml/sample_poem.xml"):a.match("event")?p("xml/event_template.xml"):a.match("letter")?p("xml/letter_template.xml"):a.match("poem")?p("xml/poem_template.xml"):
a.match("prose")?p("xml/prose_template.xml"):j.loadSchema("xml/CWRCBasicTEI.xml",!0)};var p=function(a){$.ajax({url:a,dataType:"xml",success:function(a){var b=a.createElement("rdf:RDF");$(a.childNodes?a.childNodes[a.childNodes.length-1]:a.firstChild).prepend(b);f(a)}})};return j};$.fn.filterNode=function(g){return this.find("*").filter(function(){return this.nodeName===g})};var StructureTree=function(g){var a=g.writer,b={},i=!1;$(g.parentId).append('<div id="structure"><div id="tree"></div></div>');$(document.body).append('<div id="tree_popup"></div>');$("#tree").jstree({core:{},themeroller:{},ui:{select_limit:1},json_data:{data:{data:"Tags",attr:{id:"root"},state:"open"}},contextmenu:{select_node:!0,show_at_node:!1,items:function(b){function e(b){var c={},d=!1,e;for(e in b){var d=!0,h=$("a\\:documentation, documentation",b[e].element).first(),h=1==h.length?h.text():
e;c[e]={label:'<span title="'+h+'">'+e+"</span>",icon:"img/tag_blue.png",action:function(b){var c=b.parents("li.submenu").children("a").attr("rel"),b=b.text(),d={x:parseInt($("#tree_popup").css("left")),y:parseInt($("#tree_popup").css("top"))};"change"==c?(c=$("#tree a.ui-state-active").closest("li").attr("name"),a.editor.execCommand("changeTag",{key:b,pos:d,id:c})):(a.editor.currentBookmark=a.editor.selection.getBookmark(1),a.editor.execCommand("addSchemaTag",{key:b,pos:d,action:c}))}}}d||(c.no_tags=
{label:"No tags available.",icon:"img/cross.png",action:function(){}});return c}$("#tree_popup").hide();if("root"==b.attr("id"))return{};var c=b.parents("li:first"),b=a.structs[b.attr("name")];if(b._tag==a.root||b._tag==a.header)return{};var h=a.structs[c.attr("name")],g=a.editor.execCommand("getChildrenForTag",{tag:b._tag,type:"element",returnType:"object"}),i=a.editor.execCommand("getParentsForTag",b._tag),c={};h&&(c=a.editor.execCommand("getChildrenForTag",{tag:h._tag,type:"element",returnType:"object"}));
h=e(g);i=e(i);c=e(c);c={before:{label:"Insert Tag Before",icon:"img/tag_blue_add.png",_class:"submenu",submenu:c},after:{label:"Insert Tag After",icon:"img/tag_blue_add.png",_class:"submenu",submenu:c},around:{label:"Insert Tag Around",icon:"img/tag_blue_add.png",_class:"submenu",submenu:i},inside:{label:"Insert Tag Inside",icon:"img/tag_blue_add.png",_class:"submenu",separator_after:!0,submenu:h},change:{label:"Change Tag",icon:"img/tag_blue_edit.png",_class:"submenu",submenu:c},edit:{label:"Edit Tag",
icon:"img/tag_blue_edit.png",action:function(b){var c={x:parseInt($("#tree_popup").css("left")),y:parseInt($("#tree_popup").css("top"))};a.editor.execCommand("editTag",b.attr("name"),c)}},"delete":{label:"Remove Tag",icon:"img/tag_blue_delete.png",action:function(b){a.removeStructureTag(b.attr("name"))}}};b._tag==a.root&&(delete c["delete"],delete c.before,delete c.after,delete c.around);return c}},hotkeys:{del:function(){if(this.is_selected()){var b=this.get_selected().attr("name");b&&a.removeStructureTag(b)}},
f2:!1},plugins:["json_data","ui","themeroller","contextmenu","hotkeys"]});$("#tree").mousemove(function(a){$("#tree_popup").offset({left:a.pageX+15,top:a.pageY+5})});$("#tree").bind("select_node.jstree",function(b,e){if(!i){var c=e.rslt.obj.attr("name");c&&(a.structs[c]._tag==a.header?a.d.show("header"):a.selectStructureTag(c))}i=!1});$("#tree").bind("hover_node.jstree",function(b,e){if("visible"!=$("#vakata-contextmenu").css("visibility")){var c=e.rslt.obj;if("root"!=c.attr("id")){var c=c.attr("name"),
c=a.structs[c],h="<ul>",g;for(g in c)0!=g.indexOf("_")&&(h+="<li>"+g+": "+c[g]+"</li>");g=h+"</ul>";$("#tree_popup").html(g).show()}}});$("#tree").bind("dehover_node.jstree",function(){$("#tree_popup").hide()});b.update=function(){var b=a.editor.dom.select("body");$("#tree").jstree("delete_node","#root");var e=$("#tree").jstree("create_node",$("#tree"),"first",{data:"Tags",attr:{id:"root"},state:"open"});h($(b).children(),e)};var h=function(b,e){b.each(function(){var b=$(this).children(),d=e;if($(this).attr("_tag")||
$(this).is(a.root)){var g=$(this).attr("id"),i=0<$(this).find("[_tag]").length?"open":null;$(this).attr("_tag")==a.header&&(i=!1);if(""==g||null==g){var g=tinymce.DOM.uniqueId("struct_"),l=$(this).attr("_tag");null==l&&$(this).is(a.root)&&(l=a.root);-1!=a.schema.elements.indexOf(l)&&($(this).attr("id",g).attr("_tag",l),a.structs[g]={id:g,_tag:l})}else null==a.structs[g]?(l=a.deletedStructs[g],null!=l&&(a.structs[g]=l,delete a.deletedStructs[g])):(l=a.editor.$("[id="+g+"]"),1<l.length&&l.each(function(b,
c){if(0<b){var d=$(c),e=tinymce.DOM.uniqueId("struct_");d.attr("id",e);a.structs[e]={};for(var h in a.structs[g])a.structs[e][h]=a.structs[g][h];a.structs[e].id=e}}));if(l=a.structs[g])d=l._tag,d=$("#tree").jstree("create_node",e,"last",{data:d,attr:{name:g,"class":$(this).attr("class")},state:i})}$(this).attr("_tag")!=a.header&&h(b,d)})};b.selectNode=function(a){a&&(i=!0,"tree"==$("#tree").jstree("select_node",$('#tree [name="'+a+'"]'),!0).attr("id")&&(i=!1))};return b};var EntitiesList=function(g){var a=g.writer,b={};$(g.parentId).append('<div id="entities"><div id="sortBy"><span>Sort By</span> <input type="radio" id="sequence" name="sortBy" checked="checked"><label for="sequence">Sequence</label></input><input type="radio" id="category" name="sortBy"><label for="category">Category</label></input></div><ul class="entitiesList"></ul></div>');$(document.body).append('<div id="entitiesMenu" class="contextMenu" style="display: none;"><ul><li id="editEntity"><ins style="background:url(img/tag_blue_edit.png) center center no-repeat;" />Edit Entity</li><li id="removeEntity"><ins style="background:url(img/cross.png) center center no-repeat;" />Remove Entity</li><li class="separator" id="copyEntity"><ins style="background:url(img/tag_blue_copy.png) center center no-repeat;" />Copy Entity</li></ul></div>');
$("#sequence").button().click(function(){a.entitiesList.update("sequence");a.highlightEntity(a.editor.currentEntity)});$("#category").button().click(function(){a.entitiesList.update("category");a.highlightEntity(a.editor.currentEntity)});$("#sortBy").buttonset();b.update=function(b){null==b&&(b=$("#sequence").prop("checked")?"sequence":"category");$("#entities > ul").empty();var d,e,c="",g=a.editor.$("span[class~=start]");if("category"==b){var k={};g.each(function(b,c){d=$(c).attr("name");if(null==
a.entities[d]){var g=a.deletedEntities[d];if(null!=g)e=a.entities[d]=g,delete a.deletedEntities[d];else{a.removeEntity(d);return}}else e=a.entities[d];null==k[e.props.type]&&(k[e.props.type]=[]);k[e.props.type].push(e)});var m;for(d in k){m=k[d];for(b=0;b<m.length;b++)e=m[b],c+=i(e)}}else"sequence"==b&&g.each(function(){d=$(this).attr("name");if(null==a.entities[d]){var b=a.deletedEntities[d];if(null!=b)e=a.entities[d]=b,delete a.deletedEntities[d];else{a.removeEntity(d);return}}else e=a.entities[d];
e&&(g=g.not("[name="+d+"]"),c+=i(e))});$("#entities > ul").html(c);$("#entities > ul > li").hover(function(){$(this).hasClass("selected")||$(this).addClass("over")},function(){$(this).hasClass("selected")||$(this).removeClass("over")}).mousedown(function(){$(this).removeClass("over");a.highlightEntity(this.getAttribute("name"),null,!0)}).contextMenu("entitiesMenu",{bindings:{editEntity:function(b){a.editTag($(b).attr("name"))},removeEntity:function(b){a.removeEntity($(b).attr("name"))},copyEntity:function(b){a.copyEntity($(b).attr("name"))}},
shadow:!1,menuStyle:{backgroundColor:"#FFFFFF",border:"1px solid #D4D0C8",boxShadow:"1px 1px 2px #CCCCCC",padding:"0px"},itemStyle:{fontFamily:"Tahoma,Verdana,Arial,Helvetica",fontSize:"11px",color:"#000",lineHeight:"20px",padding:"0px",cursor:"pointer",textDecoration:"none",border:"none"},itemHoverStyle:{color:"#000",backgroundColor:"#DBECF3",border:"none"}})};var i=function(a){var b="<ul>",e=function(a){for(var g in a){var h=a[g];$.isPlainObject(h)?e(h):b+="<li><strong>"+g+"</strong>: "+h+"</li>"}};
e(a.info);b+="</ul>";return'<li class="'+a.props.type+'" name="'+a.props.id+'"><span class="box"/><span class="entityTitle">'+a.props.title+'</span><div class="info">'+b+"</div></li>"};b.remove=function(a){$('#entities li[name="'+a+'"]').remove()};return b};var Relations=function(g){var a=g.writer;$(g.parentId).append('<div id="relations"><ul class="relationsList"></ul></div>');$(document.body).append('<div id="relationsMenu" class="contextMenu" style="display: none;"><ul><li id="removeRelation"><ins style="background:url(img/cross.png) center center no-repeat;" />Remove Relation</li></ul></div>');var b={update:function(){$("#relations ul").empty();for(var g="",h=0;h<a.triples.length;h++)var d=a.triples[h],g=g+("<li>"+d.subject.text+" "+d.predicate.text+
" "+d.object.text+"</li>");$("#relations ul").html(g);$("#relations ul li").each(function(a){$(this).data("index",a)}).contextMenu("relationsMenu",{bindings:{removeRelation:function(d){d=$(d).data("index");a.triples.splice(d,1);b.update()}},shadow:!1,menuStyle:{backgroundColor:"#FFFFFF",border:"1px solid #D4D0C8",boxShadow:"1px 1px 2px #CCCCCC",padding:"0px",width:"105px"},itemStyle:{fontFamily:"Tahoma,Verdana,Arial,Helvetica",fontSize:"11px",color:"#000",lineHeight:"20px",padding:"0px",cursor:"pointer",
textDecoration:"none",border:"none"},itemHoverStyle:{color:"#000",backgroundColor:"#DBECF3",border:"none"}})}};return b};